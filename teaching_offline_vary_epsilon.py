...
class teaaching:
    ...
    def offline_attack(self):
        for setting in self.settings_to_run:
            ...
            for tchr in self.teachers_to_run:
                ...
                teacher_obj = teacher.teacher(env=env_in, target_pi=target_pi, p=p, epsilon=epsilon, epsilon_p=epsilon_p, teacher_type=teacher_type, pool=pool)
    ...
    def append_cost_to_accumulator(self, cost, teacher_type, p, cost_p, success_prob, R_c):
        key = "{}_p={}_cost_p={}_R_c={}_success_prob={}".format(teacher_type, p, cost_p, R_c, success_prob)
...
if __name__ == "__main__":
    ...
    p = 0.9
    for epsilon in np.round(np.arange(0, 1.01, 0.05), 2):
    ...
    plot_chain.plot_offline_teaching_vary_eps(dict_to_plot=dict_accumulator, plot_every_after_n_points=1, show_plots=False)